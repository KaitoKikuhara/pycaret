...
class DataTypes_Auto_infer(BaseEstimator, TransformerMixin):
    ...
    def fit(self, dataset, y=None):  # learning data types of all the columns
        ...
        for i in data.drop(self.target, axis=1).columns:
            ...
            match = datefinder.find_dates(
                data[i].values[0])  # to get the first value
            try:
                for m in match:
        ...
        if self.display_types == True:
            display(wg.Text(value="Following data types have been inferred automatically, if they are correct press enter to continue or type 'quit' otherwise.",
                            layout=Layout(width='100%')), display_id='m1')

            dt_print_out = pd.DataFrame(
                self.learent_dtypes, columns=['Feature_Type'])
            ...
            dt_print_out.drop(index='dummy_target',
                              errors='ignore', inplace=True)
            # drop any columns that were asked to drop
            dt_print_out.drop(index=self.features_todrop,
                              errors='ignore', inplace=True)
    ...
    def transform(self, dataset, y=None):
        ...
        for i in self.final_training_columns:
            if i not in data.columns:
                sys.exit('(Type Error): test data does not have column ' +
                         str(i) + " which was used for training")
...
class Simple_Imputer(BaseEstimator, TransformerMixin):
    ...
    def fit(self, dataset, y=None):
        ...
        if self.numeric_strategy == 'mean':
            self.numeric_stats = data.drop(self.target, axis=1).select_dtypes(
                include=['float64', 'int64']).apply(np.nanmean)
        else:
            self.numeric_stats = data.drop(self.target, axis=1).select_dtypes(
                include=['float64', 'int64']).apply(np.nanmedian)

        self.numeric_columns = data.drop(self.target, axis=1).select_dtypes(
            include=['float64', 'int64']).columns

        # for Catgorical ,
        if self.categorical_strategy == 'most frequent':
            self.categorical_columns = data.drop(
                self.target, axis=1).select_dtypes(include=['object']).columns
            self.categorical_stats = pd.DataFrame(
                columns=self.categorical_columns)  # place holder
            for i in (self.categorical_stats.columns):
                self.categorical_stats.loc[0,
                                           i] = data[i].value_counts().index[0]
        else:
            self.categorical_columns = data.drop(
                self.target, axis=1).select_dtypes(include=['object']).columns

        # for time, there is only one way, pick up the most frequent one
        self.time_columns = data.drop(self.target, axis=1).select_dtypes(
            include=['datetime64[ns]']).columns
        self.time_stats = pd.DataFrame(
            columns=self.time_columns)  # place holder
...
class Surrogate_Imputer(BaseEstimator, TransformerMixin):
    ...
    def fit(self, dataset, y=None):
        ...
        if self.categorical_strategy == 'most frequent':
            ...
            self.categorical_na = pd.DataFrame(
                columns=self.categorical_columns)
            # (in this situation we are not making any surrogate column)
            self.categorical_na.loc[0, :] = False
...
class Catagorical_variables_With_Rare_levels(BaseEstimator, TransformerMixin):
    ...
    def fit(self, dataset, y=None):  # we will learn for what columnns what are the level to merge as others
        ...
        data = dataset.copy()
        self.ph = pd.DataFrame(columns=data.drop(
            self.target, axis=1).select_dtypes(include="object").columns)
        ...
        for i in data[self.ph.columns].columns:
            ...
            a = np.sum(pd.DataFrame(
                data[i].value_counts().sort_values())[i] <= count_th)
    ...
    def transform(self, dataset, y=None):
        ...
        for i in data[self.ph.columns].columns:
            ...
            data[i].replace(to_replace=t_replace,
                            value=self.new_level_name, inplace=True)
        return(data)
...
class New_Catagorical_Levels_in_TestData(BaseEstimator, TransformerMixin):
    ...
    def fit(self, data, y=None):
        ...
        self.ph_train_level = pd.DataFrame(columns=data.drop(
            self.target, axis=1).select_dtypes(include="object").columns)
        for i in self.ph_train_level.columns:
            if self.replacement_strategy == "least frequent":
                ...
                self.ph_train_level.loc[0, i] = list(
                    data[i].value_counts().index)

    def transform(self, data, y=None):
        # transorm
        # we need to learn the same for test data , and then we will compare to check what levels are new in there
        self.ph_test_level = pd.DataFrame(columns=data.drop(
            self.target, axis=1, errors='ignore').select_dtypes(include="object").columns)
        ...
        for i in self.ph_test_level.columns:
            ...
            if len(new) > 0:
                data[i].replace(
                    new, self.ph_train_level.loc[0, i][0], inplace=True)
    ...
    def transform(self, dataset, y=None):
        ...
        if len(self.features_to_discretize) > 0:
            data_t = self.disc.transform(
                np.array(data[self.features_to_discretize]).reshape(-1, self.len_columns))
        ...
        if len(self.numeric_features) > 0:
            if self.function_to_apply == 'zscore':
                ...
                self.scale_and_power = QuantileTransformer(
                    random_state=self.random_state_quantile, output_distribution='normal')
                self.scale_and_power.fit(data[self.numeric_features])
    ...
    def transform(self, dataset, y=None):
        ...
        for i in self.time_feature:
            ...
            if 'is_month_end' in self.list_of_features_o:
                data[i + "_is_month_end"] = [1 if calendar.monthrange(datetime.date(
                    r).year, datetime.date(r).month)[1] == datetime.date(r).day else 0 for r in data[i]]
                data[i + "_is_month_end"] = data[i +
                                                 "_is_month_end"].apply(str)
            ...
            if 'is_month_start' in self.list_of_features_o:
                ...
                data[i + "_is_month_start"] = data[i +
                                                   "_is_month_start"].apply(str)
...
class Ordinal(BaseEstimator, TransformerMixin):
    ...
    def transform(self, dataset, y=None):
        data = dataset.copy()
        new_data_test = pd.DataFrame(self.enc.transform(
            data[self.info_as_dict.keys()]), columns=self.info_as_dict.keys(), index=data.index)
    ...
    def fit_transform(self, dataset, y=None):
        ...
        new_data_train = pd.DataFrame(self.enc.fit_transform(
            data.loc[:, self.info_as_dict.keys()]), columns=self.info_as_dict, index=data.index)
...
class Dummify(BaseEstimator, TransformerMixin):
    ...
    def fit(self, dataset, y=None):
        ...
        if len(data.select_dtypes(include=('object')).columns) > 0:
            ...
            self.data_nonc = data.drop(
                self.target, axis=1, errors='ignore').select_dtypes(exclude=('object'))
            self.target_column = data[[self.target]]
            # # plus we will only take object data types
            try:
                self.data_columns = pd.get_dummies(data.drop(
                    self.target, axis=1, errors='ignore').select_dtypes(include=('object'))).columns
            except:
                self.data_columns = []
            # # now fit the trainin column
            self.ohe.fit(data.drop(self.target, axis=1,
                                   errors='ignore').select_dtypes(include=('object')))
        else:
            None
        return(None)
    ...
    def transform(self, dataset, y=None):
        ...
        if len(data.select_dtypes(include=('object')).columns) > 0:
            ...
            array = self.ohe.transform(data.drop(
                self.target, axis=1, errors='ignore').select_dtypes(include=('object'))).toarray()
    ...
    def fit_transform(self, dataset, y=None):
        ...
        if len(data.select_dtypes(include=('object')).columns) > 0:
            ...
            # now put target , numerical and categorical variables back togather
            data = pd.concat(
                (self.target_column, self.data_nonc, data_dummies), axis=1)
...
class Outlier(BaseEstimator, TransformerMixin):
    ...
    def fit_transform(self, dataset, y=None):
        ...
        if 'iso' in self.methods:
            self.iso = IForest(contamination=self.contamination,
                               random_state=self.random_state, behaviour='new')
        ...
        if 'pca' in self.methods:
            self.pca = PCA_od(contamination=self.contamination,
...
class Cluster_Entire_Data(BaseEstimator, TransformerMixin):
    ...
    def transform(self, dataset, y=None):
        ...
        predict = pd.DataFrame(
            self.k_object.predict(data_t1), index=data.index)
    ...
    def fit_transform(self, dataset, y=None):
        ...
        for k in self.ph.index:
            ...
            self.k_object = cluster.KMeans(
                n_clusters=c, init='k-means++', precompute_distances='auto', n_init=10, random_state=self.random_state)
        ...
        self.k_object = cluster.KMeans(n_clusters=self.clusters, init='k-means++',
                                       precompute_distances='auto', n_init=10, random_state=self.random_state)
        # now do fit predict
        predict = pd.DataFrame(
            self.k_object.fit_predict(data_t1), index=data.index)
        data['data_cluster'] = predict
        data['data_cluster'] = data['data_cluster'].astype('object')
...
class Reduce_Cardinality_with_Clustering(BaseEstimator, TransformerMixin):
    ...
    def fit_transform(self, dataset, y=None):
        ...
        data_pls = self.pls.fit_transform(data_t.drop(
            self.target, axis=1), data_t[self.target])[0]
        ...
        for i in self.feature:
            data_t1 = pd.DataFrame(dict(
                levels=data[i], comp1=data_pls[:, 0], comp2=data_pls[:, 1]), index=data.index)
        ...
        # Now start making clusters
            for k in self.ph.index:
                c = self.ph['clusters'][k]
                self.k_object = cluster.KMeans(
                    n_clusters=c, init='k-means++', precompute_distances='auto', n_init=10, random_state=self.random)
                self.k_object.fit(data_t1)
                self.ph.iloc[k, 1] = metrics.silhouette_score(
                    data_t1, self.k_object.labels_)
            ...
            # Now make the final cluster object
            self.k_object = cluster.KMeans(n_clusters=self.clusters, init='k-means++',
                                           precompute_distances='auto', n_init=10, random_state=self.random)
            # now do fit predict
            ...
            data[i] = data[i].replace(
                list(data_t1['levels']), data_t1['cluster'])
...
class Reduce_Cardinality_with_Counts(BaseEstimator, TransformerMixin):
    ...
    def fit_transform(self, dataset, y=None):
        ...
        self.ph_u = []
        for i in self.feature:
            data_t1 = pd.DataFrame(dict(levels=data[i].groupby(data[i], sort=False).count(
            ).index, counts=data[i].groupby(data[i], sort=False).count().values))
            u = data[i].unique()
...
class Make_NonLiner_Features(BaseEstimator, TransformerMixin):
    ...
    def transform(self, dataset, y=None):  # same application for test and train
        ...
        self.numeric_columns = data.drop(
            self.target, axis=1, errors='ignore').select_dtypes(include="float64").columns
        ...
        # dummy_all
        dummy_all = pd.concat(
            (data, ddc_power, ddc_sin, ddc_cos, ddc_tan), axis=1)
    ...
    def fit_transform(self, dataset, y=None):
        ...
        afs = Advanced_Feature_Selection_Classic(target=self.target, ml_usecase=self.ml_usecase,
                                                 top_features_to_pick=self.top_features_to_pick, random_state=self.random_state, subclass=self.subclass)
...
class Advanced_Feature_Selection_Classic(BaseEstimator, TransformerMixin):
    ...
    def fit_transform(self, dataset, y=None):
        ...
        if self.ml_usecase == 'classification':
            m = rfc(100, max_depth=5, max_features=max_fe, n_jobs=-1,
                    max_samples=max_sa, random_state=self.random_state)
        else:
            m = rfr(100, max_depth=5, max_features=max_fe, n_jobs=-1,
                    max_samples=max_sa, random_state=self.random_state)
        ...
        self.fe_imp_table = pd.DataFrame(m.feature_importances_, columns=[
                                         'Importance'], index=dummy_all.drop(self.target, axis=1).columns)
        self.fe_imp_table = self.fe_imp_table[self.fe_imp_table['Importance']
        ...
        # LightGBM
        max_fe = min(70, int(np.sqrt(len(dummy_all.columns))))
        max_sa = min(float(1000 / len(dummy_all)),
                     float(np.sqrt(len(dummy_all) / len(dummy_all))))

        if self.ml_usecase == 'classification':
            m = lgbmc(n_estimators=100, max_depth=5, n_jobs=-1,
                      subsample=max_sa, random_state=self.random_state)
        else:
            m = lgbmr(n_estimators=100, max_depth=5, n_jobs=-1,
                      subsample=max_sa, random_state=self.random_state)
        m.fit(dummy_all.drop(self.target, axis=1), dummy_all[self.target])
        # self.fe_imp_table= pd.DataFrame(m.feature_importances_,columns=['Importance'],index=dummy_all.drop(self.target,axis=1).columns).sort_values(by='Importance',ascending= False)
        self.fe_imp_table = pd.DataFrame(m.feature_importances_, columns=[
        ...
        if self.subclass != 'multi':
            ...
            corr = pd.DataFrame(
                dict(features=corr.index, value=corr)).reset_index(drop=True)
        ...
        self.selected_columns = list(set(
            [self.target] + list(dummy_all_columns_RF) + list(corr) + list(dummy_all_columns_LGBM)))

        self.selected_columns_test = dataset[self.selected_columns].drop(
            self.target, axis=1).columns
...
class Fix_multicollinearity(BaseEstimator, TransformerMixin):
    ...
    def fit(self, data, y=None):
        ...
        self.corr_matrix.values[tuple(
            [np.arange(self.corr_matrix.shape[0])] * 2)] = np.NaN
        ...
        self.targ_cor = pd.DataFrame(
            self.corr_matrix[self.target_variable].dropna())
        ...
        self.melt = self.corr_matrix.melt(id_vars=['column'], value_vars=self.cols).sort_values(
            by='value', ascending=False).dropna()

        # now bring in the avg correlation for first of the pair
        self.merge = pd.merge(self.melt, self.avg_cor, left_on='column',
                              right_on='features').drop('features', axis=1)

        # now bring in the avg correlation for second of the pair
        self.merge = pd.merge(self.merge, self.avg_cor, left_on='variable',
                              right_on='features').drop('features', axis=1)

        # now bring in the target correlation for first of the pair
        self.merge = pd.merge(self.merge, self.targ_cor, left_on='column',
                              right_on='features').drop('features', axis=1)

        # now bring in the avg correlation for second of the pair
        self.merge = pd.merge(self.merge, self.targ_cor, left_on='variable',
                              right_on='features').drop('features', axis=1)
        ...
        self.merge['all_columns'] = self.merge['column'] + \
            self.merge['variable']
        ...
        self.merge['all_columns'] = [sorted(i)
                                     for i in self.merge['all_columns']]
        ...
        self.merge['rank_x'] = round(self.multicol_weight * (self.merge['avg_cor_y'] - self.merge['avg_cor_x']) + self.target_corr_weight * (
            self.merge['target_variable_x'] - self.merge['target_variable_y']), 6)  # round it to 6 digits
        ...
        self.non_zero = self.merge[(self.merge['rank_x'] != 0.0) & (
            self.merge['value'] >= self.threshold)]

        # pick the column to delete
        self.non_zero_list = list(np.where(
            self.non_zero['rank_x'] < 0, self.non_zero['column'], self.non_zero['variable']))
        ...
        if self.correlation_with_target_threshold != 0.0:
            corr = pd.DataFrame(np.corrcoef(data.drop(self.to_drop, axis=1).T), columns=data.drop(
                self.to_drop, axis=1).columns, index=data.drop(self.to_drop, axis=1).columns)
            ...
            self.to_drop_taret_correlation = list(
                self.to_drop_taret_correlation.index)
    ...
    def transform(self, dataset, y=None):
        ...
        data.drop(self.to_drop_taret_correlation,
...
class DFS_Classic(BaseEstimator, TransformerMixin):
    ...
    def __init__(self, target, ml_usecase='classification', interactions=['multiply', 'divide', 'add', 'subtract'], top_features_to_pick_percentage=.05, random_state=42, subclass='ignore'):
        ...
        self.top_n_correlated = top_features_to_pick_percentage
        self.ml_usecase = ml_usecase
        self.random_state = random_state
        self.subclass = subclass
    ...
    def transform(self, dataset, y=None):
        ...
        if 'multiply' in self.interactions:
            ...
            data_multiply = pd.concat([data.drop(self.target, axis=1, errors='ignore').mul(
                col[1], axis="index") for col in data.drop(self.target, axis=1, errors='ignore').iteritems()], axis=1)
            data_multiply.columns = ["_multiply_".join([i, j]) for j in data.drop(
                self.target, axis=1, errors='ignore').columns for i in data.drop(self.target, axis=1, errors='ignore').columns]
        ...
        if 'divide' in self.interactions:
            ...
            data_divide = pd.concat([data.drop(self.target, axis=1, errors='ignore')[self.numeric_columns].div(
                col[1], axis="index") for col in data.drop(self.target, axis=1, errors='ignore')[self.numeric_columns].iteritems()], axis=1)
            data_divide.columns = ["_divide_".join([i, j]) for j in data.drop(self.target, axis=1, errors='ignore')[
                self.numeric_columns].columns for i in data.drop(self.target, axis=1, errors='ignore')[self.numeric_columns].columns]
        ...
        if 'add' in self.interactions:
            ...
            data_add = pd.concat([data.drop(self.target, axis=1, errors='ignore')[self.numeric_columns].add(
                col[1], axis="index") for col in data.drop(self.target, axis=1, errors='ignore')[self.numeric_columns].iteritems()], axis=1)
            data_add.columns = ["_add_".join([i, j]) for j in data.drop(self.target, axis=1, errors='ignore')[
                self.numeric_columns].columns for i in data.drop(self.target, axis=1, errors='ignore')[self.numeric_columns].columns]
        ...
        if 'subtract' in self.interactions:
            ...
            data_substract = pd.concat([data.drop(self.target, axis=1, errors='ignore')[self.numeric_columns].sub(
                col[1], axis="index") for col in data.drop(self.target, axis=1, errors='ignore')[self.numeric_columns].iteritems()], axis=1)
            data_substract.columns = ["_subtract_".join([i, j]) for j in data.drop(self.target, axis=1, errors='ignore')[
                self.numeric_columns].columns for i in data.drop(self.target, axis=1, errors='ignore')[self.numeric_columns].columns]
    ...
    def fit_transform(self, dataset, y=None):
        ...
        self.ohe_columns = [i for i in data.columns if len(data[i].unique(
        )) == 2 and data[i].unique()[0] in [0, 1] and data[i].unique()[1] in [0, 1]]
        # self.ohe_columns = [i for i in self.ohe_columns if i is not None]
        self.numeric_columns = [i for i in data.drop(
            self.target, axis=1).columns if i not in self.ohe_columns]
        ...
        if 'multiply' in self.interactions:
            data_multiply = pd.concat([data.drop(self.target, axis=1).mul(
                col[1], axis="index") for col in data.drop(self.target, axis=1).iteritems()], axis=1)
            data_multiply.columns = ["_multiply_".join([i, j]) for j in data.drop(
                self.target, axis=1).columns for i in data.drop(self.target, axis=1).columns]
            # we dont need columns that are self interacted
            col = ["_multiply_".join([i, j]) for j in data.drop(
                self.target, axis=1).columns for i in data.drop(self.target, axis=1).columns if i != j]
            ...
            col1 = [i for i in data_multiply.columns if np.nansum(
                data_multiply[i]) != 0]
        ...
        # for division, we only want it to apply to numerical columns
        if 'divide' in self.interactions:
            data_divide = pd.concat([data.drop(self.target, axis=1)[self.numeric_columns].div(
                col[1], axis="index") for col in data.drop(self.target, axis=1)[self.numeric_columns].iteritems()], axis=1)
            data_divide.columns = ["_divide_".join([i, j]) for j in data.drop(self.target, axis=1)[
                self.numeric_columns].columns for i in data.drop(self.target, axis=1)[self.numeric_columns].columns]
            # we dont need columns that are self interacted
            col = ["_divide_".join([i, j]) for j in data.drop(self.target, axis=1)[
                self.numeric_columns].columns for i in data.drop(self.target, axis=1)[self.numeric_columns].columns if i != j]
            data_divide = data_divide[col]
            # we dont need columns where the sum of the total column is null (to catagorical variables never happening togather)
            col1 = [i for i in data_divide.columns if np.nansum(
                data_divide[i]) != 0]
        ...
        # for addition, we only want it to apply to numerical columns
        if 'add' in self.interactions:
            data_add = pd.concat([data.drop(self.target, axis=1)[self.numeric_columns].add(
                col[1], axis="index") for col in data.drop(self.target, axis=1)[self.numeric_columns].iteritems()], axis=1)
            data_add.columns = ["_add_".join([i, j]) for j in data.drop(self.target, axis=1)[
                self.numeric_columns].columns for i in data.drop(self.target, axis=1)[self.numeric_columns].columns]
            # we dont need columns that are self interacted
            col = ["_add_".join([i, j]) for j in data.drop(self.target, axis=1)[
                self.numeric_columns].columns for i in data.drop(self.target, axis=1)[self.numeric_columns].columns if i != j]
        ...
        # for substraction, we only want it to apply to numerical columns
        if 'subtract' in self.interactions:
            data_substract = pd.concat([data.drop(self.target, axis=1)[self.numeric_columns].sub(
                col[1], axis="index") for col in data.drop(self.target, axis=1)[self.numeric_columns].iteritems()], axis=1)
            data_substract.columns = ["_subtract_".join([i, j]) for j in data.drop(self.target, axis=1)[
                self.numeric_columns].columns for i in data.drop(self.target, axis=1)[self.numeric_columns].columns]
            # we dont need columns that are self interacted
            col = ["_subtract_".join([i, j]) for j in data.drop(self.target, axis=1)[
                self.numeric_columns].columns for i in data.drop(self.target, axis=1)[self.numeric_columns].columns if i != j]
            data_substract = data_substract[col]
            # we dont need columns where the sum of the total column is null (to catagorical variables never happening togather)
            col1 = [i for i in data_substract.columns if np.nansum(
                data_substract[i]) != 0]
        ...
        afs = Advanced_Feature_Selection_Classic(target=self.target, ml_usecase=self.ml_usecase,
                                                 top_features_to_pick=self.top_n_correlated, random_state=self.random_state, subclass=self.subclass)
        dummy_all_t = afs.fit_transform(dummy_all)

        # self.data_fe[self.corr]
        data_fe_final = pd.concat((data, dummy_all_t), axis=1)
        # # making sure no duplicated columns are there
        # new added
...
class Reduce_Dimensions_For_Supervised_Path(BaseEstimator, TransformerMixin):
    ...
    def transform(self, dataset, y=None):
        ...
        if self.method in ['pca_liner', 'pca_kernal', 'tsne', 'incremental']:
    ...
    def fit_transform(self, dataset, y=None):
        ...
        if self.method == 'pca_liner':
            self.pca = PCA(self.variance_retained,
                           random_state=self.random_state)
            # fit transform
            data_pca = self.pca.fit_transform(
                dataset.drop(self.target, axis=1))
            data_pca = pd.DataFrame(data_pca)
            data_pca.columns = [
                "Component_" + str(i) for i in np.arange(1, len(data_pca.columns) + 1)]
            data_pca.index = dataset.index
            ...
            self.pca = KernelPCA(
                self.variance_retained, kernel='rbf', random_state=self.random_state, n_jobs=-1)
            # fit transform
            data_pca = self.pca.fit_transform(
                dataset.drop(self.target, axis=1))
            data_pca = pd.DataFrame(data_pca)
        ...
        #   return(data_pca)
            ...
            self.pca = TSNE(self.variance_retained,
                            random_state=self.random_state)
            # fit transform
            data_pca = self.pca.fit_transform(
                dataset.drop(self.target, axis=1))
...
def Preprocess_Path_One(train_data, target_variable, ml_usecase=None, test_data=None, categorical_features=[], numerical_features=[], time_features=[], features_todrop=[], display_types=True,
    ...
    global dtypes
    dtypes = DataTypes_Auto_infer(target=target_variable, ml_usecase=ml_usecase, categorical_features=categorical_features,
                                  numerical_features=numerical_features, time_features=time_features, features_todrop=features_todrop, display_types=display_types)

    # for imputation
    if imputation_type == "simple imputer":
        global imputer
        imputer = Simple_Imputer(numeric_strategy=numeric_imputation_strategy,
                                 target_variable=target_variable, categorical_strategy=categorical_imputation_strategy)
    else:
        imputer = Surrogate_Imputer(numeric_strategy=numeric_imputation_strategy,
                                    categorical_strategy=categorical_imputation_strategy, target_variable=target_variable)
    ...
    if apply_untrained_levels_treatment == True:
        ...
        new_levels = New_Catagorical_Levels_in_TestData(
            target=target_variable, replacement_strategy=untrained_levels_treatment_method)
    else:
        new_levels = Empty()
    ...
    if apply_cardinality_reduction == True and cardinal_method == 'cluster':
        cardinality = Reduce_Cardinality_with_Clustering(
            target_variable=target_variable, catagorical_feature=cardinal_features, check_clusters_upto=50, random_state=random_state)
    ...
    if apply_polynomial_trigonometry_features == True:
        global nonliner
        nonliner = Make_NonLiner_Features(target=target_variable, ml_usecase=ml_usecase, Polynomial_degree=max_polynomial, other_nonliner_features=trigonometry_calculations,
                                          top_features_to_pick=top_poly_trig_features_to_select_percentage, random_state=random_state, subclass=subcase)
    else:
        nonliner = Empty()
    ...
    if scale_data == True:
        ...
        scaling = Scaling_and_Power_transformation(
            target=target_variable, function_to_apply=scaling_method, random_state_quantile=random_state)
    else:
        scaling = Empty()
    ...
    if Power_transform_data == True:
        ...
        P_transform = Scaling_and_Power_transformation(
            target=target_variable, function_to_apply=Power_transform_method, random_state_quantile=random_state)
    else:
        P_transform = Empty()
    ...
    if remove_outliers == True:
        global rem_outliers
        rem_outliers = Outlier(target=target_variable, contamination=outlier_contamination_percentage,
                               random_state=random_state, methods=outlier_methods)
    else:
        rem_outliers = Empty()
    ...
    if cluster_entire_data == True:
        ...
        cluster_all = Cluster_Entire_Data(
            target_variable=target_variable, check_clusters_upto=range_of_clusters_to_try, random_state=random_state)
    else:
        cluster_all = Empty()
    ...
    if apply_feature_selection == True:
        ...
        feature_select = Advanced_Feature_Selection_Classic(
            target=target_variable, ml_usecase=ml_usecase, top_features_to_pick=feature_selection_top_features_percentage, random_state=random_state, subclass=subcase)
    else:
    ...
    if remove_multicollinearity == True and subcase != 'multi':
        fix_multi = Fix_multicollinearity(
            target_variable=target_variable, threshold=maximum_correlation_between_features)
    elif remove_multicollinearity == True and subcase == 'multi':
        fix_multi = Fix_multicollinearity(
            target_variable=target_variable, threshold=maximum_correlation_between_features, correlation_with_target_preference=0.0)
    else:
        fix_multi = Empty()
    ...
    if apply_feature_interactions == True:
        global dfs
        dfs = DFS_Classic(target=target_variable, ml_usecase=ml_usecase, interactions=feature_interactions_to_apply,
                          top_features_to_pick_percentage=feature_interactions_top_features_to_select_percentage, random_state=random_state, subclass=subcase)
    else:
        dfs = Empty()
    ...
    if apply_pca == True:
        ...
        pca = Reduce_Dimensions_For_Supervised_Path(
            target=target_variable, method=pca_method, variance_retained_or_number_of_components=pca_variance_retained_or_number_of_components, random_state=random_state)
    else:
        pca = Empty()
