...
def setup(data,
    ...
    if sampling is True and data.shape[0] > 25000:  # change back to 25000
        ...
        if sample_size == '' or sample_size == '1':
            ...
            X_selected, X_discard, y_selected, y_discard = train_test_split(X, y,
...
def selfsetup(_X=None, _y=None, _X_train=None, _X_test=None, _y_train=None, _y_test=None,
              _seed=None,
              _prep_pipe=None, _target_inverse_transformer=None, _experiment__=None, _preprocess=None):
    # declaring global variables to be accessed by other functions
    global X, y, X_train, X_test, y_train, y_test, seed, prep_pipe, target_inverse_transformer, experiment__, preprocess
    X = _X
    y = _y
    ...
    y_test = _y_test
    seed = _seed
    prep_pipe = _prep_pipe
    target_inverse_transformer = _target_inverse_transformer
    experiment__ = _experiment__
    preprocess = _preprocess
    if seed is None:
        import random
        seed = random.randint(150, 9000)
    if experiment__ is None:
        experiment__ = []
        import pandas as pd
        experiment__.append(('Regression Setup Config', pd.DataFrame()))
        experiment__.append(('X_training Set', X_train))
        experiment__.append(('y_training Set', y_train))
        experiment__.append(('X_test Set', X_test))
        experiment__.append(('y_test Set', y_test))
